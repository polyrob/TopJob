...
import com.threerings.challenge.event.OneTimeEvent;
		...
		if (job != null) {
			...
			if (player.getTimeInJob() % MO_JOB_INCR == 0) {
				...
				if (player.getSkills().getSkill(jobClass) < job.getMinSkills()
					...
					player.getSkills()
							.boostStat(Job.jobSkillCrossRef[jobClass]);
					td.add("You've improved your job skill, "
							+ Skills.SKILLS[Job.jobSkillCrossRef[jobClass]]);
				} else {
					td.add("This job can teach you no more about "
							+ Skills.SKILLS[Job.jobSkillCrossRef[jobClass]]);
				}
				...
				if (Rand.getSuccessForOdds(Rand.FIFTY_FIFTY)) {
					...
					td.add("Random skill bonus, " + Skills.SKILLS[randSkill]
							+ " +1");
				}
			}
		}
